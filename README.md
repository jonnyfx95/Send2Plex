...
sblocco e download è la stessa, ma ora gira come applicazione web (Blazor Server, hosting
ibrido con WebView2 per i siti che richiedono un browser reale) raggiungibile anche da telefono
sulla rete locale, con il bot Telegram ancora attivo in parallelo nello stesso processo.
...
- 🔐 **Stato login siti**: verifica se la sessione salvata su un sito è ancora valida, senza
  doverlo controllare a mano.
...
- Windows 10/11 (usa WebView2 e, per l'estrazione archivi/automazione, alcune API Windows-only)
- [.NET SDK 10](https://dotnet.microsoft.com/download) o superiore
- [WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/) (già presente su
  Windows 11 e sulla maggior parte delle installazioni Windows 10 aggiornate)
...
4. L'app parte su `http://localhost:5075` (raggiungibile anche da altri dispositivi sulla
   ...
   `Program.cs`).
5. Per i siti che richiedono login manuale (indicati in Impostazioni → Login siti): clicca
   "Login nel browser", accedi nella finestra che si apre, chiudila. Il profilo (cookie) resta
   salvato tra i riavvii in `WebView2Profile/`.
6. Da Impostazioni puoi verificare lo stato di tutti i servizi collegati ("Verifica stato") e
...
cifrate automaticamente con DPAPI la prima volta che salvi da Impostazioni (restano leggibili
solo dallo stesso utente Windows che le ha salvate).
...
se serve un browser reale per Cloudflare/JS, se serve la VPN, ecc.). Non essendo credenziali,
tecnicamente potrebbero stare nel repo — non sono incluse qui di proposito perché la scelta e
la configurazione dei siti è una decisione personale dell'utente. Campi principali di ogni voce:
...
| `UseBrowser` | `true` per siti con Cloudflare/JS (passa da WebView2 invece che da un HttpClient semplice) |
| `RequiresVpn` | Connette NordVPN automaticamente prima di cercare/scaricare su questo sito |
| `LoginUrl` | Se presente, il sito compare in Impostazioni → Login siti per il login manuale |
| `UseTorrentioApi` / `UseThePirateBayApi` | Casi speciali con API JSON dedicata invece dello scraping HTML |
...
- **Servizi** (`Services/*.cs`) — un servizio per integrazione esterna: `PlexClient`,
  `AllDebridClient`, `TmdbClient`, `TorrentSearchService`, `BrowserFetcher` (WebView2),
  `NordVpnController`, `Downloader`, `DownloadOrchestrator` (pipeline condivisa),
  `DownloadQueueService`, `DownloadHistoryService`, `LoginStatusChecker`, `TelegramWorker`
